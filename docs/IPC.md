...
| `app:elevate` | — | `boolean` | 请求 UAC 提权并以管理员重启：保存 `resumeScan` 标记 → `Start-Process -Verb RunAs` 拉起管理员实例（经 `--handover-pid` 交接：新实例轮询等旧进程退出后再取单实例锁）→ 旧实例退出；用户取消 UAC 返回 false。`AppSettings.resumeScan` 为一次性续扫标记，新实例消费后自动重扫 |
